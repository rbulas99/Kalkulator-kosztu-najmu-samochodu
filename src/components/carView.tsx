...
export const CarView = () => {
  ...
  const RENTAL_PRICE = 200; 
  const FUEL_PRICE = 7.32;
  const VAT_RATE = 1.23;
  ...
  const calculate = (car: Car) => {
    let valiDate = (Date.parse(dateTo) - Date.parse(dateFrom)) >= 0 && (Date.parse(dateFrom) >= Date.parse(currentDate));
    if(valiDate && year > 1930 && year <= 2022){
      setShowForm(false);
    if(car.type==="Premium" && year > 2018) {
    ...
    let numberOfDays = (1+ (Date.parse(dateTo) - Date.parse(dateFrom))/ (1000 * 3600 * 24));
    ...
    const costs: FinalResult = {
      ...
      fuelPrice: Math.ceil(((range/100)*(car.fuelConsumption)*(FUEL_PRICE))*100)/100,
      priceForDay: Math.ceil((result/numberOfDays)*1.23*100)/100,
  ...
  return (
    <Container>
      ...
      {showForm? <Wrapper>
        ...
        <Form>
          <ResultDetail><h3>Brutto</h3>{finalResult.brutto}PLN</ResultDetail>
          <ResultDetail><h3>Netto</h3>{finalResult.netto}PLN</ResultDetail>
          <ResultDetail><h3>Fuel Price: </h3>{finalResult.fuelPrice}PLN</ResultDetail>
          <ResultDetail><h3>Days: </h3>{finalResult.numberOfDays}</ResultDetail>
          <ResultDetail><h3>Price for day: </h3>{finalResult.priceForDay}PLN</ResultDetail>
